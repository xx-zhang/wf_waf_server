...
inline constexpr unsigned int str2tag_core(const char *s, size_t l,
                                           unsigned int h) {
    return (l == 0) ? h : str2tag_core(s + 1, l - 1,
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
    ...
    std::string response;
    {
        auto H = algo == "SHA-256" ? detail::SHA_256 : algo == "SHA-512" ? detail::SHA_512 :
                                                                           detail::MD5;
    ...
    auto field = "Digest username=\"" + username + "\", realm=\"" + auth.at("realm") + "\", nonce=\"" + auth.at("nonce") + "\", uri=\"" + req.path + "\", algorithm=" + algo + (qop.empty() ? ", response=\"" : ", qop=" + qop + ", nc=" + nc + ", cnonce=\"" + cnonce + "\", response=\"") + response + "\"" + (opaque.empty() ? "" : ", opaque=\"" + opaque + "\"");
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    if ((res.status == StatusCode::Unauthorized_401 || res.status == StatusCode::ProxyAuthenticationRequired_407) && req.authorization_count_ < 5) {
...
inline bool ClientImpl::process_request(Stream &strm, Request &req,
    ...
    if ((res.status != StatusCode::NoContent_204) && req.method != "HEAD" && req.method != "CONNECT") {
        ...
        auto out =
            req.content_receiver ? static_cast<ContentReceiverWithProgress>(
...
inline SSLSocketStream::SSLSocketStream(socket_t sock, SSL *ssl,
                                        ...
                                        time_t write_timeout_usec) :
    sock_(sock),
    ssl_(ssl), read_timeout_sec_(read_timeout_sec),
    read_timeout_usec_(read_timeout_usec),
    write_timeout_sec_(write_timeout_sec),
